...
import { selectBetsByCalendarNode, selectCalendarNodesHasBets } from '../admin/calendars/selectors';
...
const Gameweek = (): JSX.Element => {
	...
	const gameweekBets = useAppSelector(selectBetsByCalendarNode);
	...
	useEffect(() => {
		if (selectedCalendarNode) {
			setLoading(true);
			dispatch(getBetsByCalendarNode(selectedCalendarNode.id))
				.then(() => {
					setLoading(false);
				})
				.catch(() => {
