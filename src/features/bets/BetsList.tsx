								...
								{activeSeason &&
									...
									(() => {
										...
										activeSeason.leagues.forEach((l) => {
											allBets.push(...l.bets.filter((bet) => bet.betStatus === 'OPENED'));
										});
										...
										const filteredBetsByLeague =
											selectedLeagueName === 'Все'
												...
												: allBets.filter((bet) => {
														...
														return league && league.displayNameRu === selectedLeagueName;
												  });
										...
										const sortedBets = filteredBetsByPlayer.sort(
											(betA, betB) =>
												new Date(betB.createdAt).getTime() - new Date(betA.createdAt).getTime()
										);
										...
										return sortedBets.map((bet) => {
											...
											return (
												<Box key={bet.id}>{league && <BetCard bet={bet} league={league} />}</Box>
											);
										});
									})()}
							</Box>
										...
										return (
											<div>
												...
												{visibleBets < sortedBets.length && (
													<Box sx={{ display: 'flex', justifyContent: 'center' }}>
														<Button variant="contained" color="primary" onClick={showMoreBets}>
															<Typography
																variant="button"
																fontWeight="600"
																fontSize="0.9rem"
