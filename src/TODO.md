...
- страница ЧМ-2026: матчи без результатов ОТ/Пен, Плейофф без основного времени (только общий счет), Нет удобной сетки плейофф
...
- удалить кэфы прошедших матчей и не хранить их
- проверить сохранение данных ЧМ
- удалить все ненужные классы для этого ЧМ
- настроить авто календарь матчей для сезона (все 4 лиги) - отдельная АПИ
- авто подведение итогов для матчей плейофф
- резервная БК для кэфов
...
- Хочу чтобы ты убрал все fallbacks по времени начала матчей. Никаких ручных хардкод значений нигде не должно быть. Нужна только ОДНА хелпер функция которая будет преобразовывать время начала матча на основе данных utc_data/kickoff_time и других полей в корректное время выбранного региона пользователя. В настройках Мой профиль добавь такое меню и пусть информация сохраняется в accounts индивидуально для пользователя. Например для участника из Берлина своё преобразование +1 (+2 часа летом), а для москвы своё, итд. Это позволит пользователю видеть на сайте корректное для его региона время. По умолчанию пусть берутся данные из браузера если доступно.

# HOTFIX -----------------
- цвета светлой темы в логе ошибок
...
- Если будут проблемы/ошибки при массовой обработке результатов ставок - оптимизация
...
- стили светлой и темной темы: Ставки, значки лиг, текст в календаре итд...
